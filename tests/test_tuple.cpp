...
{
    SECTION("called with a function and a tuple, it forwards the call in constexpr context")
    {
        STATIC_REQUIRE(composer::apply_to(composer::minus, std::tuple(5,2)) == 3);
    }
...
TEST_CASE("apply_using", "[tuple]")
{
    SECTION("called with a function and a tuple, it forwards the call in constexpr context")
    {
        STATIC_REQUIRE(composer::apply_using(composer::minus, std::tuple(5,2)) == 3);
    }
    SECTION("called with a tuple, it binds it to the right")
    {

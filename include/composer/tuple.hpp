...
inline constexpr auto get = make_composable_function(nodiscard{
    []<typename T>(T&& t) -> decltype(std::get<I>(std::forward<T>(t))) {
        return std::get<I>(std::forward<T>(t));
    } });

inline constexpr auto apply_to = make_composable_function<back_binding>(
...
inline constexpr auto apply_using = make_composable_function<front_binding>(
    []<typename F, typename... Ts>(F&& f, Ts&&... ts)
        -> decltype(std::apply(std::forward<F>(f), std::forward<Ts>(ts)...)) {
        return std::apply(std::forward<F>(f), std::forward<Ts>(ts)...);
    });
